      ...
      appBar: PreferredSize(
        ...
        child: AppBar(
          ...
          actions: [
            ...
            //IconButton(
            //  onPressed: () {
            //      Navigator.pushNamed(context, '/account_view_page');
            //    },
            ...
            //                 Navigator.pushNamed(context, '/account_view_page');
            //               },
            //               icon: const Icon(FontAwesomeIcons.userAlt, color:kPrimaryColor, size:20.0),
            //               tooltip: 'My Profile',
            //             );
            //           } else {
            //             if (isLoggedIn == false) {
      ...
      children: [
        CustomScrollView(
          ...
          slivers: <Widget>[
            ...
            SliverToBoxAdapter(
              child: Divider(
                color: Color.fromARGB(255, 113, 98, 4),
                thickness: 5.0,
              ),
            ),
        ...
        if (total != 0)
          Positioned(
            ...
            child: Padding(
              ...
              child: InkWell(
                onTap: () {
                  if (connectList.isNotEmpty && notificationItem.isNotEmpty ||
                      connectList.isNotEmpty && refferalList.isNotEmpty ||
                      notificationItem.isNotEmpty && refferalList.isNotEmpty) {
                    _showPicker(context);
                  } else {
                    if (connectList.isNotEmpty) {
                      if (connectList[0]['pending_request'] > 0) {
                        Navigator.push(
                          ...
                          PageTransition(
                              ...
                              child: RequestListPage(
                                  userId: userId, resquestID: 0)),
                        );
                      }
                    }
                    if (refferalList.isNotEmpty) {
                      if (refferalList[0]['pending_count'] > 0) {
                        Navigator.push(
                            context,
                            PageTransition(
                                type: PageTransitionType.fade,
                    ...
                    if (notification.isNotEmpty) {
                      if (notification[0]['total_unread'] > 0) {
                        Navigator.push(
                          context,
                          PageTransition(
                              type: PageTransitionType.fade,
            ...
            Container(
              ...
              child: Column(children: [
                  ...
                  children: [
                    const Padding(
                      padding: EdgeInsets.only(
                          left: 25, right: 10.0, top: 5, bottom: 5),
                      child: Icon(
                ...
                Row(
                  children: [
                    const Padding(
                      padding: EdgeInsets.only(left: 25, right: 10.0),
                      child: Icon(
                        Icons.person,
                        color: kThirdColor,
                        size: 11,
