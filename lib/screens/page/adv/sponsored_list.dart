...
class SponsoredListPage extends StatefulWidget {
  final dynamic data;

  const SponsoredListPage({Key? key, this.data}) : super(key: key);

  @override
  _SponsoredListPageState createState() => _SponsoredListPageState();
}

class _SponsoredListPageState extends State<SponsoredListPage> {
  var userId = 0;
  List<dynamic> items = [];
      ...
      body: widget.data.isNotEmpty
          ? ListView.builder(
              ...
              itemBuilder: (BuildContext context, int index) {
                return _buildSponsoredList(items[index]);
              })
          : Center(
              child: SizedBox(
                height: 600.0,
  ...
  Widget _buildSponsoredList(dynamic data) {
        ...
        if (userId != 0) {
          if (showExpired != true) {
            Navigator.pushReplacementNamed(
                context, '/sponsored_details_view_page',
                arguments: {'data': data});
          } else {
            showDialog<String>(
                context: context,
